...
{
    if (auto v = validate_inputs(inputs, 2); !v) return fastscope::make_error<std::shared_ptr<SignalBuffer>>(v.error().code, v.error().message);
...
{
    if (auto v = validate_inputs(inputs, 1); !v) return fastscope::make_error<std::shared_ptr<SignalBuffer>>(v.error().code, v.error().message);
