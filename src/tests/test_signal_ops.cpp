...
TEST_CASE("AddOp: basic element-wise addition") {
    ...
    gnc_viz::AddOp op;
    std::array<std::shared_ptr<gnc_viz::SignalBuffer>, 2> in = {a, b};
...
TEST_CASE("AddOp: wrong input count returns error") {
    ...
    gnc_viz::AddOp op;
    std::array<std::shared_ptr<gnc_viz::SignalBuffer>, 1> in = {a};
    auto res = op.execute(in);
    REQUIRE(!res);
}
...
TEST_CASE("MagnitudeOp: 3-D magnitude") {
    ...
    gnc_viz::MagnitudeOp op;
    std::array<std::shared_ptr<gnc_viz::SignalBuffer>, 3> in = {x, y, z};
    auto res = op.execute(in);
    REQUIRE(res);
    REQUIRE_THAT((*res)->at(0), WithinAbs(5.0, 1e-9));
...
TEST_CASE("Op IDs and input counts are correct") {
    gnc_viz::AddOp      add;
    gnc_viz::SubtractOp sub;
    gnc_viz::MultiplyOp mul;
    gnc_viz::ScaleOp    scl;
    gnc_viz::MagnitudeOp mag;
